...
class Mailgun extends MailgunApi
{
	...
	{
		$this->from = Config::get('bogardo.mailgun.from');
	...
	public function send($view, array $data, $callback, $mustInit = true)
	{
		...
		return $this->mailgun()->sendMessage(Config::get('bogardo.mailgun.domain'), $this->getMessageData(), $this->getAttachmentData());
	}
	...
	{
		$data = $this->mailgun(null, Config::get('bogardo.mailgun.public_api_key'))->get("address/validate", ['address' => $address]);
	...
	public function parse($addresses, $syntaxOnly = true)
	{
		...
		$data = $this->mailgun(null, Config::get('bogardo.mailgun.public_api_key'))->get("address/parse", ['addresses' => $addresses, 'syntax_only' => $syntaxOnly]);
		return $data->http_response_body;
	}
